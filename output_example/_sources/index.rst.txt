...
The MARKUP theme is an `Attribution 3.0 Unported (CC BY 3.0) <https://creativecommons.org/licenses/by/3.0/>`__ licensed open source project that defines a theme for use with Sphinx projects.
...
#. Open the configuration file:
   ...
   and then update ``DocumentRoot`` and ``<Directory`` to have the path to the ``/output`` directory for the MARKUP theme:
